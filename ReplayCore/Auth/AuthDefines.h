...
struct sAuthLogonProof_S_BUILD_8089
{
    uint8   cmd;
    uint8   error;
    uint8   M2[20];
...
struct sAuthLogonProof_S_BUILD_6299
{
    uint8   cmd;
    uint8   error;
    uint8   M2[20];
...
struct Realm
{
    Realm(std::string name_, std::string address_, uint8 icon_, RealmFlags realmFlags_, uint8 timezone_, float populationLevel_) :
        name(name_), address(address_), icon(icon_), realmflags(realmFlags_), timezone(timezone_), populationLevel(populationLevel_) {}
    std::string name;
    std::string address;
    uint8 icon;
